...
import { RP_NAME, HASH_SALT_PREFIX, SESSION_TTL, CHALLENGE_TTL } from '../src/config/index';
...
authRouter.post(
  ...
  async (c) => {
    ...
    const { verified } = await webAuthn.verifyAuthenticationResponse({
      ...
      credential: { id: user.mfa_secret, publicKey: new Uint8Array(0), counter: 0 }, // id string (Base64URL); stubs for dev (add DB columns for prod)
    });
...
authRouter.post('/mfa-setup', async (c) => {
  ...
  const userDisplayName = result.email;
