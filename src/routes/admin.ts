...
adminRouter.post(
  ...
  async (c) => {
    ...
    if (existingDbId) {
      ...
      const stmt = await tenantDB.prepare('CREATE TABLE IF NOT EXISTS customers (id TEXT PRIMARY KEY, data JSONB NOT NULL, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);');
      await stmt.bind().run(); // No params, so empty bind; expand schema here (jobs, estimates, etc.)
    }
